<div id="the-lookup-form">
    ...
    <div id="selected-asset-info" style="display:<?php echo $phone ? 'block' :'none'; ?>;margin:5px;">
        <form method="post" class="asset" action="<?php echo $info['action'] ?  $info['action'] : '#users/lookup'; ?>">
            <input type="hidden" id="user-id" name="id" value="<?php echo $phone ? $phone->getId() : 0; ?>"/>
            <?php
            if ($phone) { ?>
                <div class="avatar pull-left" style="margin: 0 10px;">
                    <?php echo $phone->getAvatar(); ?>
                </div>
                <?php
            }
            else { ?>
            ...
            <?php }
            if ($phone) { ?>
                <div><strong id="user-name"><?php echo Format::htmlchars($phone->getName()->getOriginal()); ?></strong></div>
                <div>&lt;<span id="user-email"><?php echo $phone->getEmail(); ?></span>&gt;</div>
    ...
    <div id="new-phone-form" style="display:<?php echo $asset ? 'none' :'block'; ?>;">
        <?php if ($thisstaff->hasPerm(User::PERM_CREATE)) { ?>
            <form method="post" class="phone" action="<?php echo $info['action'] ?: '#asset/phone/lookup/form'; ?>">
                <table width="100%" class="fixed">
                    <?php
                    $form = $form ?: model\PhoneForm::getInstance();
                    $form->render(['staff' => true, 'title' => __('Create New Phone'), 'mode' => 'create']); ?>
                </table>
                <hr>
                <p class="full-width">
        <span class="buttons pull-left">
...
<script type="text/javascript">
    $(function() {
        ...
        $(document).on('click', 'form.phone input.cancel', function (e) {
            e.preventDefault();
            $('div#new-phone-form').hide();
            $('div#selected-phone-info').fadeIn({start: function(){ $('#user-search').focus(); }});
            return false;
        });
    });
</script>